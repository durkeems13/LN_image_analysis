...
def make_rgb(points,annotations,imh,imw):
    ...
    #    print(i,'sum',np.sum(np.where(a)))
    red=np.zeros([imh,imw],dtype=np.uint8)
    red[np.where(annotations[1])]=255 #4

    green=np.zeros([imh,imw],dtype=np.uint8)
    #green[np.where(annotations[0])]=255 #3
...
def main():
    ...
    parser.add_argument(
        "--read",
        type=str,
        default='../../HighRes_2ch_CD20CD3_finetune/CD20_inflamed/processing/final_overlays',
        help=""
    )
    parser.add_argument(
        "--write",
        type=str,
        default='../../HighRes_2ch_CD20CD3_finetune/CD20_inflamed/processing/final_overlays_rgb',
        help=""
    )
