...
def print_evaluation_scores(json_file):
    ...
    cocoEval.accumulate()
    cocoEval.summarize()
    fields = ['IoU=0.5:0.95', 'IoU=0.5', 'IoU=0.75', 'small', 'medium', 'large','small','medium','large']
    #for k in range(6):
    ...
    if cfg.MODE_MASK:
        ...
        cocoEval.summarize()
        for k in range(6):
            classstats=cocoEval.stats[k]
            ret['mAP(segm)/' + fields[k]+'_all'] = np.mean(classstats)
            for kkk in range(len(classstats)):
                ret['mAP(segm)/' + fields[k]+'_'+str(kkk)] = classstats[kkk]
        for k in range(9,12):
            classstats=cocoEval.stats[k]
            ret['mAR(segm)/' + fields[k-3]+'_all'] = np.mean(classstats)
            for kkk in range(len(classstats)):
                ret['mAR(segm)/' + fields[k-3]+'_'+str(kkk)] = classstats[kkk]
