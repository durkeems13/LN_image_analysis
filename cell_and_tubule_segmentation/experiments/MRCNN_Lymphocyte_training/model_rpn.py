        ...
        with tf.device('/cpu:0'):
            for th in [0.5, 0.2, 0.1]:
                ...
                pos_prediction_corr = tf.count_nonzero(
                    ...
                    dtype=tf.int32)
                #neg_prediction_corr = tf.count_nonzero(valid_label_prob < th)
                #f_pos_prediction = tf.subtract(nr_pos_prediction,pos_prediction_corr)
                ...
                #fpr = tf.to_float(tf.truediv(f_pos_prediction,tf.add(f_pos_prediction,neg_prediction_corr)))
                #fpr = tf.where(tf.equal(tf.add(f_pos_prediction,neg_prediction_corr),0),placeholder,fpr,name='fpr_th{}'.format(th))
                summaries.extend([precision, recall])#, fpr])
