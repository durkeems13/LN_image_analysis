...
def get_train_dataflow():
    ...
    aug = imgaug.AugmentorList(
        [CustomResize(cfg.PREPROC.TRAIN_SHORT_EDGE_SIZE, cfg.PREPROC.MAX_SIZE),
         imgaug.Rotation(90),imgaug.Flip(horiz=True),imgaug.Flip(vert=True)])

    def preprocess(roidb):
        fname, boxes, klass, is_crowd,scores = roidb['file_name'],roidb['boxes'],roidb['class'],roidb['is_crowd'],roidb['scores']
        boxes = np.copy(boxes)
        #im = cv2.imread(fname,cv2.IMREAD_ANYCOLOR | cv2.IMREAD_ANYDEPTH)
        im = imread(fname)
        im=im.astype(np.float32)
        im=im/np.power(2.0,16)
        images=[]
        
        for i in range(im.shape[0]):
            imge=im[i,...]*random.random()*(3.75)+.25
            images.append(imge)
        ...
        if random.random() > 0.5:
            ...
            images=[]
            for i in range(im.shape[0]):
                imge=np.power(im[i,...],random.random()*(2.5)+.25)
                images.append(imge)
        im = np.stack(images,axis=2)
        
        #im = np.stack([im[i,...] for i in range(im.shape[0])],axis=2)
        #print(im.shape)
        #print('preprocess')
    ...
    if cfg.TRAINER == 'horovod':
        ds = MultiThreadMapData(ds, 5, preprocess,buffer_size=len(ds))
        # MPI does not like fork()
    else:
        ds = MultiProcessMapDataZMQ(ds, 10, preprocess,buffer_size=len(ds))
    return ds
