...
class ResNetC4Model(DetectionModel):
    def inputs(self):
        ret = [
            tf.placeholder(tf.float32, (None, None, 4), 'image'),
...
class ResNetFPNModel(DetectionModel):
    ...
    def inputs(self):# I think we need to edit this part 
        ret = [
            tf.placeholder(tf.float32, (None, None, 4), 'image')] #RA changed from 6 to 3
        ...
        ret.extend([
            tf.placeholder(tf.float32, (None, 4), 'gt_boxes'),
            tf.placeholder(tf.int64, (None,), 'gt_labels'),  # all > 0
            tf.placeholder(tf.float32, (None, 6), 'gt_scores')])
        if cfg.MODE_MASK:
            ret.append(
                tf.placeholder(tf.uint8, (None, None, None), 'gt_masks')
            )   # NR_GT x height x width
    ...
    def build_graph(self, *inputs):
        ...
        gt_boxes, gt_labels, gt_scores = inputs['gt_boxes'],inputs['gt_labels'], inputs['gt_scores']
        if is_training:
            proposals = sample_fast_rcnn_targets(proposal_boxes, gt_boxes,gt_labels, gt_scores)
        else:
            proposals = BoxProposals(proposal_boxes)
