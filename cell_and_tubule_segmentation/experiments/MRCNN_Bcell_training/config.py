...
# dataset -----------------------
_C.DATA.BASEDIR = '../../data/MP_CD20_finetuning_trainset'
_C.DATA.TRAIN = ['train']   # i.e., trainval3k
...
_C.TRAIN.BASE_LR = 0.001 # 1e-2 defined for a total batch size of 8. Otherwise it will be adjusted automatically
...
#_C.TRAIN.LR_SCHEDULE = [2000,4000,8000,12000,20000]    # "2x" schedule in detectron
_C.TRAIN.LR_SCHEDULE = [10000,17500,20000] #for training [100000,17500,200000] for finetuning [10000,17500,20000] 
_C.TRAIN.EVAL_PERIOD = 25  # period (epochs) to run eva

# preprocessing --------------------
# Alternative old (worse & faster) setting: 600, 1024
_C.PREPROC.TRAIN_SHORT_EDGE_SIZE = [300, 512]  # [min, max] to sample from
_C.PREPROC.TEST_SHORT_EDGE_SIZE = 400
_C.PREPROC.MAX_SIZE = 667
...
_C.RPN.ANCHOR_SIZES = (8, 16, 32, 64, 128)   # sqrtarea of the anchor box
...
_C.RPN.BATCH_PER_IM = 128 #256  # total (across FPN levels) number of anchors that are marked valid
_C.RPN.MIN_SIZE = 0
_C.RPN.PROPOSAL_NMS_THRESH = 0.7
_C.RPN.CROWD_OVERLAP_THRESH = 0.7  # boxes overlapping crowd will be ignored.
_C.RPN.HEAD_DIM = 512 #1024      # used in C4 only
...
_C.FRCNN.BBOX_REG_WEIGHTS = [10., 10., 5., 5.]  # standard setting: [10, 10, 5, 5]
...
_C.FPN.FRCNN_CONV_HEAD_DIM = 128 #256
_C.FPN.FRCNN_FC_HEAD_DIM = 512 #1024
_C.FPN.MRCNN_HEAD_FUNC = 'maskrcnn_up4conv_head'   # choices: maskrcnn_up4conv_{,gn_}head

# Mask-RCNN
_C.MRCNN.HEAD_DIM = 128 #256
...
_C.TEST.RESULT_SCORE_THRESH = 0.1
_C.TEST.RESULT_SCORE_THRESH_VIS = 0.3  
